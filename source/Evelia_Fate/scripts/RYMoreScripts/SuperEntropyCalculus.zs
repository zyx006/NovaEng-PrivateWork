...
function STdispose(item as IIngredient[],output as IIngredient,jsq as int){
    RecipeBuilder.newBuilder("STdispose"+jsq,"SuperentropyCalculus",100)
    .addPreCheckHandler(function(event as RecipeCheckEvent){
            if(isNull(event.controller.recipeThreadList[1].activeRecipe)){
            event.setFailed("尚未启动超熵演算！");
            return;
...
STdispose([<ore:ingotManasteel> * 2,<ore:ingotDraconium> * 2,<ore:ingotCrystallineAlloy> * 6,<ore:ingotOsmium>,<ore:ingotGold>,<ore:ingotIron>,<ore:ingotCopper>,<ore:ingotTin>], <contenttweaker:universalalloyt1> * 2,  jsq); jsq += 1; // 基础通用合金
STdispose([<ore:ingotAlloyT1>,<ore:ingotCrystallineAlloy> * 8,<thermalfoundation:material:134> * 12],<additions:novaextended-blue_alloy_ingot> * 6,  jsq); jsq += 1; // 蓝晶合金
...
STdispose([<ore:dustCoal>,<ore:dustFluix>,<ore:ingotIron>],<threng:material>,  jsq); jsq += 1;//福鲁伊克斯钢
